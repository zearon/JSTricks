...
$ROOTPATH_PREFIX = $_SERVER["DOCUMENT_ROOT"]; 
$ROOTPATH = "$ROOTPATH_PREFIX/CloudStorage"; // Can be chagned to whatever directory on your server as you wish
...
// Authenticate
$PRIVATEKEY = md5($PASSPHASE);  //The length of key must be 16 or 32. Here a 32-length key is generated by get MD5 of the passphrase
	...
	try {
		...
		// List files
			...
			if($files = @scandir($path, 1)) {
				...
				$return["code"] = 101; // dir not exists
				$return["message"] = 'Directory does not exist. Please save a file in that directory first to create that directory.';
			}
			$return["result"] = $files;
		}
