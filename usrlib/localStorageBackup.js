	...
	function createDialog() {
		...
		$('body').append('<div id="__UI_dialog__DIV" title="Local Storage Backup and Restore">'+
			...
			'<a id="__UI_dialog__link" download="backup.txt" style="display:none">Download</a><hr/>'+
	...
	function backup() {
		...
		for(item in localStorage) {
			data += 'localStorage["' + item + '"]="' + localStorage[item].replace(/"/g, '\\"') + '"; \n';
		}
		var link = $('#__UI_dialog__link');
		link.attr('href', "data:text/plain;charset=UTF-8,"+encodeURIComponent(data));
		link.attr('data-downloadurl', "text/plain:backup.txt:"+"http://html5-demos.appspot.com/static/a.download.html");
		link.innerHtml = "Download";
		link.show();
	}
	
	function restore() {
		file = $('#__LocalStorageFP__')[0].files[0];
